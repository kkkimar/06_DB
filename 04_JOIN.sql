...
--> 컬럼이 16개가 아니라 15개인 이유
	--> 컬럼 이름이 같아서 두 테이블의 같은 컬럼이 포개지듯이 함쳐짐 

--SQL Error [918] [42000]: ORA-00918: 열의 정의가 애매합니다
--> 작성된 컬럼이 어떤 테이블이 컬럼인지 구분할 수 없음

-- 오라클 -> 별칭 사용
-- 테이블 별로 별칭을 등록할 수 있음.
SELECT EMP_ID,EMP_NAME, E.JOB_CODE
FROM EMPLOYEE E, JOB J
--WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE; --> 별칭으로 가능
...
--> 하동운, 이오리 누락됨
--> 왜? DEPT_CODE(NULL) 값과
--     같은 값이 DEPT_ID 컬럼에 존재하지 않아서 연결이 안됨--> 최종 결과에서 제외 == INNER JOIN
