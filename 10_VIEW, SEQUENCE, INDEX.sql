...
GRANT CREATE VIEW TO KH_KAR;

-- ORA-01031: 권한이 불충분합니다
--> 관리자 계정 접속해서 권한을 부여한 후 만들기
CREATE VIEW V_EMP
...
-- NEXTVAL 호출시 마다 INCREMENT BY에 작성된 값만큼 증가
...
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '홍길동'); --130
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '김영희'); --135
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '박철수'); --140

SELECT * FROM TB_TEST;

--TB_TEST 테이블 UPDATE 시 시퀀스 사용하기

--TB_TEST에서 TEST_NO 컬럼값이 가장 작은 컬럼 값을 
--다음 시퀀스 번호로 변경
...
SELECT * FROM EMPLOYEE; -- INDEX 사용 X

SELECT * FROM EMPLOYEE -- INDEX 사용 O
WHERE EMP_ID !=0;

--> 인덱스를 사용했으나 데이터가 적어서 구분 X
...
-- 인덱스 사용 O
SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO = 500000; -- 0.002S --> 속도 훨씬 빠름 (인덱스를 사용하는 이유)
