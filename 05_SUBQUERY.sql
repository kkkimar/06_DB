...
FROM EMPLOYEE
NATURAL JOIN JOB --자연 조인 : 컬럼명, 타입이 일치하는 컬럼끼리 연결
...
WHERE LOCATION_ID =(
	SELECT LOCAL_CODE  
	FROM LOCATION
	WHERE NATIONAL_CODE = 'KO' -- 다중행 서브쿼리
);
...
FROM EMPLOYEE MAIN -- 메인쿼리 테이블 별칭
WHERE SALARY > (
	SELECT AVG(SALARY)
	FROM EMPLOYEE SUB -- 서브쿼리 테이블 별칭
 ...
 * 3) 메인 쿼리에서 반환 받은 값을 이용해 해당 행의 결과 포함 여부를 결정
 * */
...
FROM (
	...
	SELECT EMP_NAME,SALARY
	FROM EMPLOYEE 
	ORDER BY SALARY DESC --> 조회결과가 메인 쿼리의 테이블 역할
)
WHERE ROWNUM <=5;
...
WHERE ROWNUM BETWEEN 1 AND 10; --> 1부터 포함해야된다! (2부터 시작하면 실패!)
 ...
 * 
 *    EX) 다른 테이블이여도 컬럼명이 같을 때 --> 테이블별로 별칭 지정
 * 																 --> 테이블명.컬럼명	
 * 
