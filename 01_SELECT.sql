...
-- EMPLOYEE 테이블에서 
-- 모든 행의 이름(EMP_NAME), 입사입(HIRE_DATE) 조회
...
--SYSTIMESTAMP : 현재 시간( 년,월,일,시,분,초,ms+ 지역(local) )
...
/* 날짜 끼리 연산하기*/
-- 날짜 - 날짜 = 일 단위 숫자
-- 1 == 1일차이 , 1.5 == 1일 12시간 차이


-- TO_DATE('문자열', '패턴') :
-- '문자열'을 '패턴'형태로 해석해서 DATE 타입으로 변경해 주는 함수

-- '' : 문자열 리터럴 표기법
SELECT TO_DATE('2024-02-26', 'YYYY-MM-DD') FROM DUAL;

-- 오늘 - 어제 (24시간 차이) ==1
SELECT TO_DATE('2024-02-26', 'YYYY-MM-DD') - TO_DATE('2024-02-25', 'YYYY-MM-DD')
FROM DUAL;

-- 현재 시간 - 어제 0시 0분 0초 = 1.52931712962962962962962962962962962963
...
SELECT CEIL (( SYSDATE - TO_DATE('1989-12-15', 'YYYY-MM-DD')) /365 ) AS 나이
FROM DUAL;
...
-- 컴럼명 자리에 작성한 리터럴 값이 추가된다

SELECT EMP_NAME, SALARY, '원' , SYSDATE 
FROM EMPLOYEE;
...
-- 조회 결과 집합(RESULT SET)의
-- 지정된 컬럼의 값이 중복되는 경우 이를 한번만 표시할 때 사용 
...
SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE
WHERE EMP_NAME LIKE '전%돈'; -- 가운데가 뭐가 들어가든 상관 없음
...
ORDER BY DEPT_CODE; --ASC가 기본값이라 생략 가능!

/* 컬럼 순서를 이용해 정렬하기 */

--EMPLOYEE 테이블에서 
--급여가 300만 이상, 600이하인 사원의
...
SELECT EMP_NAME ,PHONE FROM EMPLOYEE
ORDER BY PHONE NULLS LAST; --NULLS LAST :생략 가능 (기본값)

-- 내림차순 + NULLS FIRST (NULL인 경우 제일 위에)
SELECT EMP_NAME ,PHONE FROM EMPLOYEE
ORDER BY PHONE DESC NULLS FIRST; -- 정렬기준 -> NULL위치 순서대로 해석
