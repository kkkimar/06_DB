...
--ㅁ 문자열에서 5번부터 검색 시작해서 처음 찾은 'B'의 위치 조회
SELECT 'AABAACAABBAA', INSTR('AABAACAABBAA','B',5) 
FROM DUAL; --9

--ㅁ 문자열에서 5번부터 검색 시작해서 2번째로 찾은 'B'의 위치 조회
...
--> YEAR, MONTH 단위 계산 시 더 정확한 값을 얻어 낼 수 있다!
...
-- /, (), :, - => 일반적으로 날짜 표기시 사용하는 기호
--> 패턴으로 인식되어 오류가 발생하지 않음!

-- 년,월,일 같은 한글 또는 날짜와 관련 없는 문자는 패턴 X
--> 이러한 관련 없는 문자들을 ""로 감싸면 사용 가능!
--("" 의미 : 있는 그대로 인식)

--현재날짜 -> 2024년 02월 27일 오후 12시 15분 30초
...
SELECT TO_DATE('02월 27일 화요일 12시 24분','MM"월" DD"일" DY"요일" HH"시" MI"분"') 
FROM DUAL;
...
/*3*/SELECT EMP_ID ,EMP_NAME ,SALARY,
	CASE 
		...
		WHEN SALARY >= 3000000 THEN '중급' -- 300만 이상 500만 이하 자동으로 설정
		ELSE '초급'
	END 구분
/*1*/FROM EMPLOYEE
...
FROM EMPLOYEE                --1) EMPLOYEE 테이블 23행중
...
FROM EMPLOYEE;
-- 7행이 아닌 6행만 나옴
--> 왜? COUNT()내에 컬럼명을 작성하면 NULL은 제외됨
